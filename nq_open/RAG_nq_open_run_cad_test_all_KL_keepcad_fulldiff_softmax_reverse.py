...
if __name__ == "__main__":
    ...
    output_path = "outputs/rag_nq_" +str(num_retrieved_docs)+ "_cad_" + args.model_name.split("/")[-1] + "_short_" + str(ans_pos) + "_cadall_kl_keep_cad_fulldiff_sf_reverse.json"
    ...
    for cad_prompts, full_prompt in tqdm(zip(input_prompts, full_input_prompts)):
        ...
        for iter_gen in range(max_ans_len_total):
            ...
            full_generated_ids, _ = next_single_tok_gen(full_prompt, generated_token_accum, max_ans_len=max_ans_len)
            full_score = softmax_weight(full_generated_ids['scores'][0][0].cpu())
            ...
            for tensor, weight in zip(score_list, weights):
                weighted_sum += ((1+alpha) * tensor - alpha * empty_score) * weight
